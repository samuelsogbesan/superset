...
test('formatPermissionLabel preserves underscores in permission and view menu names', () => {
  // Underscores in catalog/schema/table names must be kept so that the
  // label matches the underscore-based value used for searching.
  expect(formatPermissionLabel('datasource_access', '[db].[data_prod]')).toBe(
    'datasource_access [db].[data_prod]',
  );

...
test('formatPermissionLabel preserves underscores in permission and view menu names', () => {
  // Underscores must be kept so labels match what the backend stores and so
  // client-side search (e.g. "data_prod") filters correctly.
  expect(
    formatPermissionLabel('can_read', 'schema access on [db].[data_prod]'),
  ).toBe('can_read schema access on [db].[data_prod]');
  expect(formatPermissionLabel('database_access', 'my_database')).toBe(
    'database_access my_database',
  );
});

test('fetchPermissionOptions keeps underscores so results match the search term', async () => {
  ...
  const label = result.data[0].label as string;
  expect(label).toBe('schema_access [examples].[data_prod]');
  // The rendered label still contains the raw search term, so AsyncSelect's
  // client-side filter does not drop backend matches.
  expect(label.includes('data_prod')).toBe(true);
});

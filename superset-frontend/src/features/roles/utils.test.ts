...
test('formatPermissionLabel preserves underscores in permission and view menu names', () => {
  // Underscores must be kept so labels match the backend representation.
  // Replacing them with spaces breaks the AsyncSelect client-side filtering
  // (e.g. searching "data_prod" would never match a "data prod" label).
  expect(formatPermissionLabel('can_read', 'data_prod')).toBe(
    'can_read data_prod',
  );
  expect(
    formatPermissionLabel('datasource access', '[db].[schema_a].[t_b]'),
  ).toBe('datasource access [db].[schema_a].[t_b]');
});

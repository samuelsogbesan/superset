...
test('formatPermissionLabel preserves underscores so labels match backend names', () => {
  // Underscores in schema/table names must be kept intact; otherwise the
  // AsyncSelect client-side filter (which matches against the label) would
  // drop results the backend returned for an underscore search term.
  expect(formatPermissionLabel('database_access', '[main].[data_prod]')).toBe(
    'database_access [main].[data_prod]',
  );
  expect(formatPermissionLabel('can_read', 'ChartView')).toBe(
    'can_read ChartView',
  );
});

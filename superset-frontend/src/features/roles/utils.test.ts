...
test('formatPermissionLabel preserves underscores in permission and view menu names', () => {
  // Underscores must be kept so the label matches what the backend stores and
  // what users type when searching (e.g. schema names like "data_prod").
  expect(formatPermissionLabel('datasource_access', '[main].[data_prod]')).toBe(
    'datasource_access [main].[data_prod]',
  );
});

test('fetchPermissionOptions returns labels searchable by the underscore term', async () => {
  ...
  // The label keeps underscores so the client-side Select filter (which matches
  // the search string against the label) still surfaces the backend results.
  expect(result.data).toEqual([
    { value: 42, label: 'datasource_access [main].[data_prod]' },
  ]);
  expect(result.data[0].label).toContain('data_prod');
});
